...
Wait for response. Store as `persona.skills: list[str]`.

Only offer skills already available in the local Bastion deployment. Assign the
confirmed identifiers to the persona; never download or install a missing skill
as a side effect of persona creation.
...
| `skills` | `list[str]` | Skills locais atribuídas a esta persona |
...
```json
{
  ...
  "skills": ["github-integration", "code-review-helper"],
  "voice_tone": "técnico, direto, com exemplos de código quando relevante",
...
- Skills assigned to a persona are recorded in `personas/{slug}/skills.json` in addition to the SOUL.md frontmatter.
